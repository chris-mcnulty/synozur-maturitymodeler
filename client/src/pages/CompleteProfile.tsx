...
export default function CompleteProfile() {
  ...
  // Fetch tenant settings to determine which fields to collect
  const { data: tenant } = useQuery<any>({
    queryKey: ['/api/user/tenant'],
    enabled: !!user,
  });

  // When tenant doesn't collect full profile data, only job title is personal
  const jobTitleOnly = tenant?.collectProfileData === false;
          ...
          <Card className="w-full max-w-lg p-8">
            <div className="mb-8 text-center">
              ...
              <h2 className="text-2xl font-bold mt-2">
                {jobTitleOnly ? "One Last Thing" : "Complete Your Profile"}
              </h2>
              <p className="text-muted-foreground mt-2">
                {jobTitleOnly
                  ? `Welcome, ${user.name || user.email}! Just tell us your role so we can personalize your experience.`
            ...
            <form name="orion-complete-profile" onSubmit={handleSubmit} className="space-y-4">
              ...
              >
                {updateProfileMutation.isPending ? (
                  ...
                  jobTitleOnly ? "Get Started" : "Complete Profile"
                )}
              </Button>
              
              <p className="text-xs text-center text-muted-foreground mt-4">
                {jobTitleOnly
                  ? "Your organization's profile details have already been filled in for you."
      ...
      <div className="flex-1 bg-gradient-to-br from-primary/10 via-primary/5 to-background p-8 hidden lg:flex flex-col justify-center">
        <div className="max-w-lg">
          <h2 className="text-3xl font-bold mb-6">
            {jobTitleOnly ? "Welcome to the Platform" : "Why We Need This Information"}
          </h2>
          <ul className="space-y-4 text-muted-foreground">
            <li className="flex items-start">
              ...
              <div>
                ...
                <p>Your role helps us tailor recommendations specifically for your context and seniority level.</p>
              </div>
            </li>
            <li className="flex items-start">
              <span className="text-primary mr-3 mt-1">
