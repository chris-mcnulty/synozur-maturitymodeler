...
- **Multi-Tenant Private Model Visibility**: ✅ **FULLY OPERATIONAL** - Models can be published as public (visible to all) or tenant-private (visible only to owning tenant members + global admins). All model access endpoints enforce visibility checks via centralized `canAccessModel` helper returning 404 for unauthorized access. Critical bug fix (Nov 2024): `getAllModels()` now correctly includes `visibility`, `owner_tenant_id`, and `model_class` columns in database queries, preventing private models from appearing public. Comprehensive automated regression tests verify: anonymous users blocked from private models, same-tenant users can access their tenant's private models, cross-tenant users blocked from other tenants' private models, and global admins can access all models. Admin panel includes visibility selector and tenant dropdown for model creation/editing. "Tenant Private" badges display on model cards. Backend uses new four-tier role system exclusively (no legacy admin/modeler role checks remain). Security verified via manual and automated testing covering all access scenarios including users with null tenant assignments.
- **Multi-Tenant Architecture (In Progress)**: Platform is transforming into a multi-tenant system serving as the OAuth 2.0 identity provider for the Synozur ecosystem (Orion, Nebula, Vega). Completed: tenant-private model visibility system with full access control enforcement. Remaining: tenant-specific branding (logo, colors), domain-based tenant mapping, application entitlements per tenant, and support for individual skills assessments. Users can exist with or without tenant association. See [MULTI_TENANT_ARCHITECTURE.md](./MULTI_TENANT_ARCHITECTURE.md) for detailed specification and [PRODUCT_BACKLOG.md](./PRODUCT_BACKLOG.md) for implementation roadmap.
