...
-   **Planner Multi-Tenant Credentials**: Each tenant can store its own Azure AD app registration credentials (`supportPlannerTenantId`, `supportPlannerClientId`, `supportPlannerClientSecret`) for Planner access. Falls back to global `PLANNER_TENANT_ID`/`PLANNER_CLIENT_ID`/`PLANNER_CLIENT_SECRET` env vars if per-tenant credentials aren't set. MSAL instances are cached per tenant+client combination. Client secret is never returned to the frontend — only a `supportPlannerHasClientSecret` boolean.
