...
-   **Planner Multi-Tenant Integration**: Reuses the existing multi-tenant Entra ID app registration (AZURE_CLIENT_ID/AZURE_CLIENT_SECRET) — no per-tenant credentials needed. Each tenant's `ssoTenantId` (set automatically when users SSO in) is used as the MSAL authority. When an admin grants consent for the Orion app, both SSO and Planner permissions (Tasks.ReadWrite.All, Group.Read.All) are included. MSAL instances are cached per Azure AD tenant. No separate PLANNER_* env vars required.
...
-   **Microsoft Graph / Planner**: Optional ticket-to-task sync, uses same Entra app as SSO (`AZURE_CLIENT_ID`, `AZURE_CLIENT_SECRET`) with each tenant's `ssoTenantId` for authority. Requires `Tasks.ReadWrite.All` and `Group.Read.All` application permissions via admin consent. Uses `@azure/msal-node` for app-only auth.
