...
export class ObjectStorageService {
  ...
  /**
   * Persist server-generated bytes (e.g. TTS narration audio, rendered slide
   * images) under the private bucket and return the canonical `/objects/<id>`
   * path served by `GET /objects/:objectPath`. `entityId` is the path *within*
   * the private dir, e.g. `narration/<uuid>.mp3`.
   */
  async storeObjectBytes(opts: {
    entityId: string;
    data: Buffer;
    contentType: string;
    acl: ObjectAclPolicy;
    ...
    const fullPath = `${privateDir}/${opts.entityId}`;
    const { bucketName, objectName } = parseObjectPath(fullPath);
